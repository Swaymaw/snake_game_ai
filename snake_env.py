...
class SnakeEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.observation_space = spaces.Box(low=-1, high=500,
                                            shape=(7+SNAKE_LEN_GOAL,), dtype=np.int64)
    ...
    def step(self, action):
        ...
        if (self.apple_position[0] - 5 <= self.snake_head[0] <= self.apple_position[0] + 5) and (self.apple_position[1] - 5 <= self.snake_head[1] <= self.apple_position[1] + 5):
        ...
        observation = [head_x, head_y, tail_x, tail_y, apple_x, apple_y, snake_length] + list(self.prev_actions)
